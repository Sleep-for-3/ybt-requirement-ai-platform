...
def build_project_overview(db: Session, project_id: int) -> dict:
    ...
    business_evidence = _distinct_mapping_count(db, "scenario_business", select(ScenarioBusinessMapping.id).where(ScenarioBusinessMapping.project_id == project_id))
    technical_evidence = _distinct_mapping_count(db, "scenario_technical", select(ScenarioTechnicalLineage.id).where(ScenarioTechnicalLineage.project_id == project_id))
    ...
    metrics = {
        "readiness_score": _score_metric("readiness_score", readiness["score"], readiness["score"], "当前项目准备度维度", as_of),
...
def _score_metric(metric_code: str, numerator: float, denominator: float, scope: str, as_of: datetime) -> dict:
    payload = _metric_payload(metric_code, numerator, denominator, as_of, scope=scope)
    payload["value"] = numerator
    return payload

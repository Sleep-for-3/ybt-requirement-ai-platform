...
class SemanticConceptVersion(Base, TimestampMixin):
    """Canonical, governed meaning for one stable SemanticConcept identity."""

    __tablename__ = "semantic_concept_versions"
    __table_args__ = (
        UniqueConstraint(
            "semantic_concept_id", "version_no",
            name="uq_semantic_concept_version",
        ),
        CheckConstraint(
            "effective_to IS NULL OR effective_to >= effective_from",
            name="ck_semantic_concept_version_dates",
        ),
        Index("ix_semantic_concept_version_project_status", "project_id", "status"),
        Index("ix_semantic_concept_version_concept_status", "semantic_concept_id", "status"),
        Index("ix_semantic_concept_version_project_effective", "project_id", "effective_from", "effective_to"),
    )
    ...
    business_domain: Mapped[str | None] = mapped_column(String(200), index=True)
    owner_department: Mapped[str | None] = mapped_column(String(200))
    provenance_json: Mapped[dict] = mapped_column(MutableDict.as_mutable(JSON), default=dict)
    status: Mapped[str] = mapped_column(String(30), default="draft", nullable=False, index=True)
    confidence_level: Mapped[str] = mapped_column(String(30), default="medium", nullable=False)
    source_type: Mapped[str] = mapped_column(String(50), default="manual", nullable=False)
    source_id: Mapped[int | None] = mapped_column(Integer)

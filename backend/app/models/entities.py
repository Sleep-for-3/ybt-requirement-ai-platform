...
class ModelProfile(Base,TimestampMixin):
    __tablename__="model_profiles";id:Mapped[int]=mapped_column(Integer,primary_key=True);profile_name:Mapped[str]=mapped_column(String(255),unique=True);provider_type:Mapped[str]=mapped_column(String(50));base_url:Mapped[str|None]=mapped_column(String(500));model_name:Mapped[str|None]=mapped_column(String(255));embedding_model_name:Mapped[str|None]=mapped_column(String(255));api_key_env_name:Mapped[str|None]=mapped_column(String(100));enabled:Mapped[bool]=mapped_column(Boolean,default=True);local_only:Mapped[bool]=mapped_column(Boolean,default=False);supports_structured_output:Mapped[bool]=mapped_column(Boolean,default=True);max_context_tokens:Mapped[int]=mapped_column(Integer,default=8192);temperature:Mapped[float]=mapped_column(Float,default=.2);config_json:Mapped[dict]=mapped_column(MutableDict.as_mutable(JSON),default=dict);created_by:Mapped[str|None]=mapped_column(String(100))
...
class RagEvaluationCase(Base,TimestampMixin):
    __tablename__="rag_evaluation_cases";id:Mapped[int]=mapped_column(Integer,primary_key=True);project_id:Mapped[int]=mapped_column(ForeignKey("projects.id"),index=True);institution_id:Mapped[int|None]=mapped_column(ForeignKey("institutions.id"),index=True);case_name:Mapped[str]=mapped_column(String(255));case_type:Mapped[str]=mapped_column(String(50));query_text:Mapped[str]=mapped_column(Text);target_field_id:Mapped[int|None]=mapped_column(ForeignKey("target_fields.id"));scenario_id:Mapped[int|None]=mapped_column(ForeignKey("product_scenarios.id"));expected_knowledge_unit_ids_json:Mapped[list]=mapped_column(MutableList.as_mutable(JSON),default=list);expected_source_system:Mapped[str|None]=mapped_column(String(255));expected_table_name:Mapped[str|None]=mapped_column(String(255));expected_field_name:Mapped[str|None]=mapped_column(String(255));expected_answer_keywords_json:Mapped[list]=mapped_column(MutableList.as_mutable(JSON),default=list);source_feedback_id:Mapped[int|None]=mapped_column(ForeignKey("ai_user_feedback.id"),unique=True,index=True);model_call_log_id:Mapped[int|None]=mapped_column(ForeignKey("model_call_logs.id"),index=True);target_type:Mapped[str|None]=mapped_column(String(50));target_id:Mapped[int|None]=mapped_column(Integer);execution_kind:Mapped[str|None]=mapped_column(String(30));output_hash:Mapped[str|None]=mapped_column(String(64),index=True);input_context_json:Mapped[dict|None]=mapped_column(JSON,nullable=True);expected_output_json:Mapped[dict|None]=mapped_column(JSON,nullable=True);assertions_json:Mapped[list]=mapped_column(MutableList.as_mutable(JSON),default=list);enabled:Mapped[bool]=mapped_column(Boolean,default=True);created_by:Mapped[str|None]=mapped_column(String(100))

class RagEvaluationRun(Base):
    __tablename__="rag_evaluation_runs";id:Mapped[int]=mapped_column(Integer,primary_key=True);project_id:Mapped[int]=mapped_column(ForeignKey("projects.id"),index=True);institution_id:Mapped[int|None]=mapped_column(ForeignKey("institutions.id"),index=True);run_name:Mapped[str]=mapped_column(String(255));model_profile_id:Mapped[int|None]=mapped_column(ForeignKey("model_profiles.id"));retrieval_config_json:Mapped[dict]=mapped_column(MutableDict.as_mutable(JSON),default=dict);status:Mapped[str]=mapped_column(String(50));started_at:Mapped[object|None]=mapped_column(DateTime(timezone=True));finished_at:Mapped[object|None]=mapped_column(DateTime(timezone=True));summary_metrics_json:Mapped[dict]=mapped_column(MutableDict.as_mutable(JSON),default=dict);created_at:Mapped[object]=mapped_column(DateTime(timezone=True),server_default=func.now());created_by:Mapped[str|None]=mapped_column(String(100))

class RagEvaluationResult(Base):
    __tablename__="rag_evaluation_results";id:Mapped[int]=mapped_column(Integer,primary_key=True);evaluation_run_id:Mapped[int]=mapped_column(ForeignKey("rag_evaluation_runs.id"),index=True);evaluation_case_id:Mapped[int]=mapped_column(ForeignKey("rag_evaluation_cases.id"),index=True);retrieved_unit_ids_json:Mapped[list]=mapped_column(MutableList.as_mutable(JSON),default=list);generated_answer:Mapped[str|None]=mapped_column(Text);citations_json:Mapped[list]=mapped_column(MutableList.as_mutable(JSON),default=list);execution_metadata_json:Mapped[dict|None]=mapped_column(JSON,nullable=True);recall_at_k:Mapped[float]=mapped_column(Float,default=0);reciprocal_rank:Mapped[float]=mapped_column(Float,default=0);source_hit:Mapped[bool]=mapped_column(Boolean,default=False);citation_coverage:Mapped[float]=mapped_column(Float,default=0);groundedness_score:Mapped[float]=mapped_column(Float,default=0);keyword_coverage:Mapped[float]=mapped_column(Float,default=0);latency_ms:Mapped[int]=mapped_column(Integer,default=0);error_message:Mapped[str|None]=mapped_column(Text);created_at:Mapped[object]=mapped_column(DateTime(timezone=True),server_default=func.now())

class AIUserFeedback(Base):
    __tablename__="ai_user_feedback";id:Mapped[int]=mapped_column(Integer,primary_key=True);project_id:Mapped[int]=mapped_column(ForeignKey("projects.id"),index=True);institution_id:Mapped[int|None]=mapped_column(ForeignKey("institutions.id"),index=True);model_call_log_id:Mapped[int|None]=mapped_column(ForeignKey("model_call_logs.id"),index=True);feedback_type:Mapped[str]=mapped_column(String(50));target_type:Mapped[str]=mapped_column(String(50));target_id:Mapped[int]=mapped_column(Integer);rating:Mapped[str]=mapped_column(String(50));correct_source_system:Mapped[str|None]=mapped_column(String(255));correct_table_name:Mapped[str|None]=mapped_column(String(255));correct_field_name:Mapped[str|None]=mapped_column(String(255));comment:Mapped[str|None]=mapped_column(Text);output_hash:Mapped[str|None]=mapped_column(String(64),index=True);execution_kind:Mapped[str|None]=mapped_column(String(30),index=True);execution_metadata_json:Mapped[dict|None]=mapped_column(JSON,nullable=True);created_at:Mapped[object]=mapped_column(DateTime(timezone=True),server_default=func.now());created_by:Mapped[str|None]=mapped_column(String(100))

class ModelCallLog(Base):
    __tablename__="model_call_logs";id:Mapped[int]=mapped_column(Integer,primary_key=True);project_id:Mapped[int]=mapped_column(ForeignKey("projects.id"),index=True);model_profile_id:Mapped[int|None]=mapped_column(ForeignKey("model_profiles.id"));retrieval_log_id:Mapped[int|None]=mapped_column(ForeignKey("retrieval_logs.id"),index=True);prompt_key:Mapped[str]=mapped_column(String(100));prompt_version:Mapped[int]=mapped_column(Integer);skill_key:Mapped[str|None]=mapped_column(String(100),index=True);skill_version:Mapped[str|None]=mapped_column(String(50),index=True);provider:Mapped[str|None]=mapped_column(String(50),index=True);model_name:Mapped[str|None]=mapped_column(String(255),index=True);execution_kind:Mapped[str|None]=mapped_column(String(30),index=True);request_hash:Mapped[str]=mapped_column(String(64));context_hash:Mapped[str|None]=mapped_column(String(64),index=True);context_complete:Mapped[bool|None]=mapped_column(Boolean,nullable=True);context_budget_json:Mapped[dict|None]=mapped_column(JSON,nullable=True);input_summary:Mapped[str|None]=mapped_column(Text);output_summary:Mapped[str|None]=mapped_column(Text);output_hash:Mapped[str|None]=mapped_column(String(64),index=True);citations_json:Mapped[list|None]=mapped_column(JSON,nullable=True);rejected_claims_json:Mapped[list|None]=mapped_column(JSON,nullable=True);execution_metadata_json:Mapped[dict|None]=mapped_column(JSON,nullable=True);status:Mapped[str]=mapped_column(String(50),index=True);latency_ms:Mapped[int]=mapped_column(Integer);token_usage_json:Mapped[dict]=mapped_column(MutableDict.as_mutable(JSON),default=dict);confidentiality_level:Mapped[str]=mapped_column(String(50));error_type:Mapped[str|None]=mapped_column(String(100));http_status:Mapped[int|None]=mapped_column(Integer);error_detail:Mapped[str|None]=mapped_column(Text);created_at:Mapped[object]=mapped_column(DateTime(timezone=True),server_default=func.now());created_by:Mapped[str|None]=mapped_column(String(100))

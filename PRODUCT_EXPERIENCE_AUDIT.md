...
审计日期：2026-08-31
代码基线：本地提交 `31117be`；`origin/main` 已核对为 `b31faea8e3b5f20fea36a736d0290edf1aeebb7d`
范围：监管数据智能治理平台的核心用户旅程、信息架构、语言、状态反馈、AI 可解释性和运行时可信度。

## 方法与边界

本次审计以业务分析、技术分析、审核人员、监管负责人和平台管理员五类 Persona 的任务完成为主线，结合前端路由/组件、后端 API 合同、自动化测试和既有真实 Runtime 报告。没有扩充 Product 5.1 Demo 数据集，没有修改 Golden Truth、监管业务定义或权限政策，没有引入第二套 AI Framework。

本轮使用现有 `scripts/项目启停.ps1 start -Mode sqlite -SkipDataMigration` 尝试启动，后端进程启动但 `/health/ready` 持续返回 503，日志明确记录 `embedding_dimension_missing`；脚本最终超时退出，前端端口也未开放。因此新增 `/work` 和任务详情上下文没有声称通过认证浏览器 UAT；此前 Admin/Cockpit 修复的浏览器证据保留在 `P0_PRODUCT_INTEGRITY_FIX_REPORT.md`。Real Agent 基准沿用 `PRODUCT_5_1_REAL_BENCHMARK.md` 的诚实结论：`REAL_AGENT_BENCHMARK_PARTIAL`。

## Findings

| ID | Module | Persona | Severity | Problem / Business Impact | Evidence / Root Cause | Recommended Fix | Status |
|---|---|---|---|---|---|---|---|
| PE-P0-01 | Real Agent Benchmark | 业务分析、技术分析 | P0 | 17 字段仅 10 个完整成功、7 个 partial；不能作为全量监管口径或正式交付依据。 | `PRODUCT_5_1_REAL_BENCHMARK.md`：第三方长响应 `RemoteProtocolError` / `invalid_model_response`。 | 先解决 provider 长请求稳定性、上下文压缩和异步任务超时，再重跑失败字段；保持失败可见，禁止补造结果。 | Open |
| PE-P1-01 | 我的工作 / Review | 审核人员 | P1 | `/review-tasks` 与 `/tasks` 曾造成“我的待办”和“安全查询”语义漂移，打开任务后返回上下文不稳定。 | `navigation-contract.mjs` 原先把 `/review-tasks` 标为“我的待办”，详情路由固定归属 `/tasks`。 | `/work` 作为 canonical 我的工作；`/review-tasks` 兼容；从工作中心打开任务携带 `from=work` 与完整 return state。 | Implemented; browser pending |
...
| PE-P1-04 | Requirement Workspace | 业务分析、技术分析 | P1 | 用户需要同时理解监管定义、Evidence、Semantic、Source/Mart 候选和 AI 置信度；只展示 AI 结果会降低信任。 | `DocumentPreview` 已具备这些数据，但缺少统一解释入口。 | 增加“为什么这样判断？”结构化面板，不重复调用 AI，默认展示业务语言，技术标识渐进展开。 | Implemented |
| PE-P1-05 | Requirement editing | 业务分析、技术分析 | P1 | 核心口径编辑若在切字段、刷新或请求失败时静默丢失，会直接造成返工和监管风险。 | `SelectedFieldEditor`/`RequirementWorkspace` 已有 dirty、saving、saved、error、beforeunload 和切换确认。 | 继续以服务器为正式事实；补认证浏览器验证网络失败、刷新和跨字段切换。 | Implemented; browser pending |
...
| PE-P2-07 | Impact story | 技术分析、监管负责人 | P2 | 影响链若主要呈现 JSON 或内部节点，用户无法快速回答发生了什么、谁要处理、截止何时。 | 后端 SQL→Source→Mart→Target→Requirement→ReviewTask 链已成立；前端仍有技术化表达。 | 保留现有链，增加业务故事摘要、风险、负责人、动作 deep link；原始 JSON 只放技术详情。 | Open |
...
字段场景页已经能把监管原始定义、细化定义、Evidence、Semantic、Source/Mart 候选和人工保存放在同一工作区；AI Explanation 让“为什么这样判断”可追溯。当前阻塞是 7 个真实 Agent partial、语义绑定仍有 `ai_suggested`，以及尚未完成认证浏览器下的切换/网络失败验证。

### 技术分析人员

Source → Mart → YBT 线索和 SQL transformation 可在工作区查看，Impact 后端已传播到 Target、Requirement、ReviewTask。Impact 的业务故事化 UI、负责人和 action deep link 仍是 P2。

### 审核人员

`/work` 列表提供待处理、临近/超期、退回、搜索、状态筛选和 URL 恢复；详情页提供上下文、历史决定和通过/退回/驳回操作，驳回/退回要求原因。业务对象名称、风险和等待时间尚未全部由后端摘要提供；认证浏览器 UAT 待服务启动后执行。

### 监管负责人

Cockpit 已按服务端 capability 和真实项目范围工作，支持 partial/system error 区分；但 Real Agent partial、语义未确认、Quality 未执行和正式 Deliverable 未就绪仍必须在驾驶舱明确呈现，不能用漂亮数字掩盖。
...
本轮低风险产品改进已集中在“我的工作”、任务上下文、统一产品语言、搜索上下文和 AI Explanation；没有扩大后端领域模型。最大未关闭风险仍是 Real Agent Benchmark 的 7 个 partial 字段和认证环境验证缺口。下一步应先稳定真实 provider 并完成语义/人工治理，再做 Impact Story、Recent/Favorite 和更大范围视觉收敛。

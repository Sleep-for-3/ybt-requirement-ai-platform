...
> 验证基线：本地 Product 5.1 工作分支 `2c25ea8b97066b51da1a2512412bd5584593ab4b`（远端 `origin/main` 当前为 `b31faea8e3b5f20fea36a736d0290edf1aeebb7d`）
>
...
> 数据生成种子：`20260830`
>
> 本手册记录的是 2026-08-31 本地真实运行与浏览器 UAT 结果。
...
它不是普通 AI 文档生成器：AI 只能在受治理的 RegulatoryContext 上工作；AI Draft 与 Human Final 分离；结果必须能回到监管条目、数据字段和 SQL。当前真实运行时已经接通，17 字段基准结果为 `REAL_AGENT_BENCHMARK_PASS`。该状态表示真实 Agent 结果和后验评估完成，不表示语义、映射已经人工批准或正式交付已放行。
...
已验证的链路包括数据构建、7 个数据源注册与连接测试、元数据同步、Catalog、Knowledge、Semantic、Target/Scenario、双层 Mapping 任务、SQL 解析、Lineage、Impact、Reporting Cycle、Cockpit 和真实 Agent。Quality Execution 与正式 Deliverable 仍是后续 release gate。
...
10. Evidence：监管知识已经索引，RegulatoryContext 对重点字段返回 30–32 条 Knowledge Evidence；真实 Agent 的 Evidence Citation 已在 17 个字段结果中记录。
11. Lineage：进入 `/lineage`，看到 2 份脚本、节点列表和未完成影响；再进入 `/lineage/scripts` 查看 Source→Mart v1 和 Mart→YBT v2。Target resolver 已支持 bootstrap 记录的物理字段元数据；重点字段可查询 Target→Mart→Source，仍有 69 个节点按分类待治理。
12. Review：真实 Agent 已生成 17 个结构化字段结果；这不等于正式口径。AI Draft→Human Edit→Business Review→Technical Review→Final 仍需按字段完成治理，Impact workflow 的 3 个任务也不代表字段审核已完成。
...
15. Impact：进入 `/lineage/changes/3` 查看 v1→v2 critical diff，再进入 `/lineage/impacts/40` 查看真实影响传播、业务故事和 3 个待办；原始 SQL/内部标识位于技术详情。
16. Dashboard：进入 `/cockpit`，机构驾驶舱按服务端 capability 展示授权范围内项目及治理风险。错误会区分登录失效、无权限、系统故障、网络故障和部分数据不可用。
...
7. 打开 `/cockpit`：“驾驶舱显示真实治理状态和授权范围；项目级分析异常只影响对应行，不会打死整个机构视图。”
8. 收尾：“真实 Runtime 和 17 字段 Agent Benchmark 已完成；结果仍是 AI Draft，语义确认、质量执行和正式交付必须继续由人工治理。”
...
9. 用 1 分钟展示 `/cockpit` 的准备度和风险；说明 partial/system error 的真实错误体验。
10. 用最后 1 分钟总结已验证能力、`REAL_AGENT_BENCHMARK_PASS`、产品成熟度和下一阶段关闭项。
...
| LLM Runtime | `REAL_AGENT_BENCHMARK_PASS` | provider=`openai_compatible`，model=`gpt-5.6-sol`；真实调用与幂等复用记录见 Benchmark 报告 |
| Embedding / Vector | FastEmbed 512 维 / Milvus | 非 mock；collection `ybt_semantic_p5_v5_ec2e880fd3f8_d512`，32 vectors |
| AI Mapping / AI Requirement | 17 success / 0 partial | 17 字段结构化结果完成；Agent 成功不等于人工批准 |
| Target→Mart Accuracy | 17/17 (100%) | 17 字段完成后离线后验匹配 |
| Mart→Source Accuracy | 15/17 (88%) | 低命中来源继续人工治理 |
| Transformation Accuracy | 10/17 (59%) | 低命中转换继续人工治理 |
| Evidence Citation Accuracy | 17/17 (100%) | 字段级证据引用可匹配 |
| Semantic Match Accuracy | 17/17 (100%) | 仅作离线匹配；ai_suggested 不等于已批准 |
| Hallucinated Asset Count | 0（complete heuristic） | partial 字段不乐观归零，需人工复核 |
| Missing Required Source Count | 7（complete heuristic） | 不把缺失来源乐观归零 |
...
Agent Requirement、AI Evidence Citation 和 Human Final：Agent Requirement 与 Evidence Citation 已在真实运行中产生；Human Final 需按治理 workflow 确认。Golden Mapping 只在 Agent 完成后用于后验评估，不是 AI 成果，也没有进入 Agent Prompt。
...
因此“SQL 变化→Source→Mart→Target Field→Semantic→Regulatory Requirement→审核任务”的传播链已在重新运行的真实 Impact 中成立；3 个 ReviewTask 仍表示需要人工处理，并不代表审核已完成。
...
5. Metric/Cockpit 使用治理服务的真实统计口径；驾驶舱按授权范围展示项目状态，不使用截图硬编码。
6. Action 是人工治理和 ReviewTask。AI 只生成 Draft；业务、技术和最终审核决定正式口径。当前真实 Agent 已完成，但仍没有声称正式口径闭环已完成。
...
| Queue | Celery 5 + Redis 7 为生产路径 | production 启动路径使用 Redis + Celery；sqlite 模式才使用 inline |
...
| LLM | OpenAI-compatible、本地 vLLM/Ollama-compatible provider | `openai_compatible / gpt-5.6-sol`；JSON Mode 使用现有 `json_mode=false` + validator |
...
| Analytics | Metric Registry、Reporting Cycle、Project Analytics、Institution Cockpit | Cockpit 与核心 Dashboard 可用；Quality Execution 仍未实现 |
...
- 真实 Runtime 下 17 个字段均有结构化 Agent 结果；早期 provider 错误和幂等复用记录均保留在 benchmark artifact，不把 Agent 结果包装成人工批准口径。
...
| ID | 现象与影响 | 复现 | 推荐修改 |
| --- | --- | --- | --- |
| P0-01（已缓解） | 早期真实运行有 7 个 partial，不能把不完整结果当正式口径。 | 历史 benchmark artifact 记录 `RemoteProtocolError` / `invalid_model_response`；后续 Sol 运行完成 17/17。 | 保持 bounded retry 和上下文约束；对 Golden 低命中项继续人工治理，不修改 Golden Truth。 |
...
| ID | 现象与影响 | 复现 | 推荐修改 |
| --- | --- | --- | --- |
| P1-01（已修复） | Project Dashboard 在未登录或单个接口异常时可能显示空白/无限加载。 | 未登录访问 `/projects/1/dashboard`；此前会停在加载态。 | 已加入会话门禁、超时、独立错误分类和部分数据容错；核心 production bundle 路径已用认证会话复验。 |
| P1-02（已修复） | SQL Impact 历史快照曾停在 Mart/Mapping/Semantic，Target Field 与 Requirement 为 0。 | 旧 impact 快照可复现。 | Target resolver 已支持物理列元数据；重新运行 v1→v2 后 impact 40 已传播到 17 Target、17 Requirement。 |
| P1-03（已修复核心路径） | 旧重点 Target Field lineage API 曾返回 0 node/edge，而原始脚本图存在链路。 | 历史 verification 查询。 | resolver 回归测试和重新 ingestion 后重点字段可查询 Target→Mart→Source；69 个非核心 unresolved 节点按分类保留。 |
...
| P1-06（逻辑已修复，历史事件保留） | 重复 Metadata Sync 历史上累积过 Schema Drift 事件。 | 查看历史 Demo verification。 | deterministic repeated-sync 第二次 drift=0；不删除历史事件，后续继续监测新同步。 |
...
理由：核心领域模型、数据源、Catalog、Knowledge、Semantic、Lineage、Impact、Cockpit、权限和治理框架已经形成可运行产品，真实 LLM、Embedding、Milvus 已接通且 17 字段 Agent Benchmark 完成；但语义/人工内容闭环覆盖、Quality Execution、Deliverable、完整多角色浏览器 UAT 和 staging release gates 仍未完成，因此尚不能称为 UAT Ready 或 Production Candidate。

## 17. 下一阶段建议

1. 对 Golden 低命中来源与转换完成候选资产和人工治理校准，不修改 Golden Truth。
2. 在 staging 与认证 UAT 中继续复验 Target Field lineage identity 和 v1→v2 Impact 传播。
3. 扩大 Semantic/Mapping 人工确认覆盖，再比较 RegulatoryContext trusted facts 的变化。
4. 将 ReviewTask 摘要补齐业务名称、风险、负责人和等待时长，并统一全站状态 contract。
5. 持续审计 Schema Drift 去重逻辑和 69 个 unresolved node。
6. 实现可审计 Quality Execution 或继续明确标注“Expectation only”；不得将规则数量当作质量分。
7. 配置正式 Deliverable Template，等 Agent 与审核闭环完成后再验证交付包。
8. 在更多真实角色和桌面视口完成认证浏览器 UAT、备份恢复、安全、性能和 driver matrix release gates。

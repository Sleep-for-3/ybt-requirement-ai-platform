...
16. Dashboard：进入 `/cockpit`，机构驾驶舱可用，显示 1 个项目及治理风险。项目 Dashboard 已修复未登录时的无限加载/空白、Dashboard 与 Analytics 的部分加载和错误分类；当前没有认证浏览器会话，因此 authenticated 200 payload UAT 仍未宣称通过。

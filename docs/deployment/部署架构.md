...
知识文件由 API 持久化后投递到 Redis，Celery Worker 解析文件并以固定批次写入 PostgreSQL。API 不在上传请求内执行解析，因此大文件索引期间前端仍可切换页面和调用其他接口；任务进度写回 `BackgroundJob.progress/current_step` 供页面轮询。

...
数据库 Profile 引用的环境变量会优先从运行进程读取；本地进程未注入时，再从 `backend/.env` 按变量名读取。修改 `.env` 后需要重启项目。使用透明代理/TUN 时，DNS 可能把公网模型域名映射到 `198.18.0.0/15` Fake-IP 网段；运行时允许公网域名解析到这一专用代理网段，但仍拒绝直接填写 Fake IP、回环地址、普通私网、链路本地地址和云元数据地址。

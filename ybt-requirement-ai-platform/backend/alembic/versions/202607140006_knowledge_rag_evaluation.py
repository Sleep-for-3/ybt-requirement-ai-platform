...
from app.models import (AIUserFeedback,EmbeddingRecord,KnowledgeDocumentVersion,KnowledgeEntityLink,KnowledgeIngestionTask,KnowledgeKeywordIndex,KnowledgeUnit,ModelCallLog,ModelProfile,PromptTemplateVersion,RagEvaluationCase,RagEvaluationResult,RagEvaluationRun,RetrievalLog)
...
def upgrade():
    bind=op.get_bind()
    for table in [KnowledgeDocumentVersion.__table__,KnowledgeUnit.__table__,KnowledgeKeywordIndex.__table__,KnowledgeEntityLink.__table__,KnowledgeIngestionTask.__table__,EmbeddingRecord.__table__,RetrievalLog.__table__,ModelProfile.__table__,PromptTemplateVersion.__table__,RagEvaluationCase.__table__,RagEvaluationRun.__table__,RagEvaluationResult.__table__,AIUserFeedback.__table__,ModelCallLog.__table__]:table.create(bind=bind,checkfirst=True)
    for name,column in {
        "knowledge_type":sa.Column("knowledge_type",sa.String(50),server_default="manual_note"),"knowledge_scope":sa.Column("knowledge_scope",sa.String(50),server_default="project"),"institution_name":sa.Column("institution_name",sa.String(255)),"document_status":sa.Column("document_status",sa.String(50),server_default="pending"),"confidentiality_level":sa.Column("confidentiality_level",sa.String(50),server_default="internal"),"file_hash":sa.Column("file_hash",sa.String(64)),"current_version_no":sa.Column("current_version_no",sa.Integer,server_default="1"),"parse_status":sa.Column("parse_status",sa.String(50),server_default="pending"),"parse_summary_json":sa.Column("parse_summary_json",sa.JSON,server_default="{}"),"warnings_json":sa.Column("warnings_json",sa.JSON,server_default="[]"),"error_message":sa.Column("error_message",sa.Text),"created_by":sa.Column("created_by",sa.String(100)),
        # SQLite rejects ALTER TABLE ADD COLUMN with CURRENT_TIMESTAMP as a
        # non-constant default. Application writes still populate updated_at.
        "updated_at":sa.Column("updated_at",sa.DateTime(timezone=True))}.items():_add("knowledge_documents",name,column)
...
def downgrade():
    ...
    with op.batch_alter_table("knowledge_documents") as batch:
        for column in knowledge_document_columns:
            batch.drop_column(column)
    for name in ["ModelCallLog","AIUserFeedback","RagEvaluationResult","RagEvaluationRun","RagEvaluationCase","PromptTemplateVersion","ModelProfile","RetrievalLog","EmbeddingRecord","KnowledgeIngestionTask","KnowledgeEntityLink","KnowledgeKeywordIndex","KnowledgeUnit","KnowledgeDocumentVersion"]:

...
def run_column_profile(db:Session,column:CatalogColumn,request,created_by=None):
    ...
    try:
        ...
        if "min_max" in metrics and sensitivity not in {"sensitive","highly_sensitive"}:queries.append(("min_max",f"select min({column_sql}) as min_value, max({column_sql}) as max_value from {table_sql}"))
        elif "min_max" in metrics:warnings.append("敏感字段已跳过 min/max 原值")
        ...
        if "top_values" in metrics and sensitivity=="normal" and distinct<=1000:
            ...
            warnings.extend(response.warnings)
            if response.status=="success" and response.rows and all("value" in row for row in response.rows):result["top_values"]=response.rows
        ...
        result,sanitization_warnings=sanitize_profile_result(result,sensitivity);warnings.extend(sanitization_warnings)

...
def search_catalog(db: Session, project_id: int, request):
    ...
    for column, table, datasource in db.execute(statement.limit(5000)).all():
        ...
        if score <= 0: continue
        bindings = list(db.scalars(select(CatalogImportBinding).where(CatalogImportBinding.catalog_column_id == column.id)).all())
        results.append({"catalog_column_id":column.id,"datasource_id":datasource.id,"datasource_name":datasource.name,"schema_name":column.schema_name,"table_name":column.table_name,"table_comment":table.table_comment,"column_name":column.column_name,"column_comment":column.column_comment,"data_type":column.data_type,"nullable":column.nullable,"is_primary_key":column.is_primary_key,"score":score,"match_reasons":reasons,"imported_source_field_id":next((x.source_field_id for x in bindings if x.binding_type=="source_field"),None),"imported_mart_field_id":next((x.mart_field_id for x in bindings if x.binding_type=="mart_field"),None)})
    return sorted(results,key=lambda x:(x["score"],-x["catalog_column_id"]),reverse=True)[:request.top_k]
...
def import_source_table(db, table, request):
    ...
    source=db.scalar(select(SourceTable).where(SourceTable.project_id==table.project_id,SourceTable.business_system_id==system.id,SourceTable.table_code==table.table_name))
...
def import_mart_column(db,column):
    table=db.get(CatalogTable,column.catalog_table_id); mart=db.scalar(select(MartTable).where(MartTable.project_id==table.project_id,MartTable.table_code==table.table_name))

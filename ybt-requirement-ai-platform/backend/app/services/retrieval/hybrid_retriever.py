...
class HybridRetriever:
    ...
    def search(self,project_id,query,target_field_id=None,scenario_id=None,knowledge_types=None,top_k=20,created_by=None):
        ...
        visibility=or_(and_(KnowledgeUnit.knowledge_scope=="project",KnowledgeUnit.project_id==project_id),KnowledgeUnit.knowledge_scope=="global",and_(KnowledgeUnit.knowledge_scope=="institution",KnowledgeUnit.institution_name==project.bank_name));predicates=[KnowledgeUnit.enabled.is_(True),visibility]
        if knowledge_types:predicates.append(KnowledgeUnit.knowledge_type.in_(knowledge_types))
        if scenario_id:predicates.append(or_(KnowledgeUnit.scenario_id==scenario_id,KnowledgeUnit.scenario_id.is_(None)))
        tokens=tokenize(" ".join(filter(None,[query,target.field_code if target else None,target.field_name if target else None,target.field_definition if target else None])))
        candidate_ids=[]
        if tokens:
            ranked=select(KnowledgeKeywordIndex.knowledge_unit_id,func.sum(KnowledgeKeywordIndex.weight).label("keyword_weight")).join(KnowledgeUnit,KnowledgeUnit.id==KnowledgeKeywordIndex.knowledge_unit_id).where(*predicates,KnowledgeKeywordIndex.token.in_(tokens)).group_by(KnowledgeKeywordIndex.knowledge_unit_id).order_by(func.sum(KnowledgeKeywordIndex.weight).desc()).limit(max(top_k*20,500))

...
本轮同时建立可持续监管知识库：`.xlsx`、`.docx`、`.pdf`、`.txt`、`.md`、`.sql` 会解析为保留 sheet、单元格、标题或页码的 `KnowledgeUnit`，经结构化过滤、关键词和向量混合检索，为来源推荐、场景口径和双层口径提供真实 citation。知识支持项目/银行/全局作用域、文件哈希版本去重、软删除、敏感等级、RAG 评测和用户反馈。

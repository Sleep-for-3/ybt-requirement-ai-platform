      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-5 lg:px-6">
        <div className="min-w-0">
          <nav aria-label="当前位置" className="mb-2 flex min-w-0 items-center gap-1.5 text-xs text-slate-500">
            {trail.sectionHref !== "/resources" ? <Link className="hover:text-pine-700" href="/workspace">工作台</Link> : null}
            {trail.sectionHref && trail.sectionHref !== "/workspace" ? <>{trail.sectionHref !== "/resources"?<ChevronRight aria-hidden size={13} />:null}<Link className="truncate hover:text-pine-700" href={trail.sectionHref}>{trail.sectionLabel}</Link></> : null}
            {trail.parentHref && trail.parentHref !== trail.sectionHref ? <><ChevronRight aria-hidden size={13} /><Link href={parentHref || trail.parentHref}>{trail.parentLabel || "上级"}</Link></> : null}
          ...
          {parentHref ? <Link className="button-ghost -ml-2 mb-2 h-8 px-2 text-xs" href={parentHref}><ArrowLeft aria-hidden size={15} />返回{trail.parentLabel || trail.sectionLabel || "上级"}</Link> : null}

...
export function AsyncRegion<T>({ label, state, emptyText, onRetry, children }: {
  ...
  if (kind === "forbidden") return <section className="rounded-lg border border-gold-200 bg-gold-50 p-5" role="alert"><h2 className="text-base font-semibold text-gold-900">无权查看{label}</h2><p className="mt-2 text-sm text-gold-800">当前账号可继续查看语义概览，但没有此区域的资源权限。</p></section>;
  if (kind === "error") return <section className="rounded-lg border border-coral-200 bg-coral-50 p-5" role="alert"><h2 className="text-base font-semibold text-coral-800">{label}加载失败</h2><p className="mt-2 text-sm text-coral-700">该区域未加载，不代表没有数据。</p>{state.error?.message ? <p className="mt-1 text-xs text-coral-700">{state.error.message}</p> : null}<button aria-label={`重试加载${label}`} className="button-secondary mt-4 min-h-11" onClick={onRetry} type="button"><RotateCw aria-hidden size={15} />重试加载{label}</button></section>;
...
function RegionSkeleton({ label }: { label:string }) {
  return <section aria-busy="true" aria-label={`正在加载${label}`} className="space-y-3"><div aria-hidden className="h-5 w-40 animate-pulse rounded bg-slate-200" />{[0, 1, 2].map((item) => <div aria-hidden className="h-16 animate-pulse rounded-lg border border-line bg-white" key={item} />)}</section>;
}

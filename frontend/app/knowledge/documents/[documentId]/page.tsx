...
import { Ban, CheckCircle2, PlayCircle, RefreshCw } from "lucide-react";
...
type DocumentVersion={id:number;version_no:number;file_name:string;parse_status:string;lifecycle_status?:string;regulatory_version?:string|null;internal_revision?:string|null;publisher?:string|null;published_at?:string|null;effective_at?:string|null;expires_at?:string|null;replaces_version_id?:number|null;change_note?:string|null;reviewed_by?:string|null;reviewed_at?:string|null;activated_at?:string|null};
...
export default function Page() {
  ...
  async function govern(version:DocumentVersion, action:"review"|"activate"){
  ...
  return (
    <main>
      <WorkspaceHeader title={doc?.file_name || "知识文档"} meta={`${knowledgeLabel(doc?.document_status)} / ${units.length} 个知识单元 / ${doc?.current_version_id?`当前生效内部版本 ${doc.current_version_no}`:"尚无生效版本"}`} />
      <div className="mx-auto max-w-5xl space-y-4 p-4 lg:p-6">
        ...
        {tab==="版本记录"?<section className="panel p-5"><h2 className="mb-2 font-semibold">版本记录</h2><p className="mb-4 text-sm text-slate-500">监管版本与内部修订版本分别展示；解析失败或未激活版本不会替换当前问答语料。</p>{versions.some(v=>v.parse_status==="failed")&&doc?.current_version_id?<p className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm">存在解析失败的新版本；问答仍在使用内部版本 {doc.current_version_no}。</p>:null}{versions.length?<ul className="divide-y">{versions.map(v=><li className="grid gap-3 py-4 md:grid-cols-[1fr_1fr_auto] md:items-center" key={v.id}><div><strong>内部版本 {v.version_no}</strong><p className="mt-1 text-sm text-slate-500">监管版本：{v.regulatory_version||"未提供"} · 内部修订：{v.internal_revision||`R${v.version_no}`}</p><p className="mt-1 text-sm">{v.file_name} · {knowledgeLabel(v.parse_status)} · <span className={v.lifecycle_status==="active"?"badge-success":v.lifecycle_status==="pending_review"?"badge-warning":"badge-neutral"}>{LIFE[v.lifecycle_status||""]||"历史导入"}</span></p></div><div className="text-sm text-slate-600"><p>发布机构：{v.publisher||doc?.publisher||"未提供"}</p><p>生效日期：{v.effective_at?new Date(v.effective_at).toLocaleDateString("zh-CN"):"未提供"}</p><p className="mt-1 break-words">变更说明：{v.change_note||"未填写"}</p></div><div className="flex flex-wrap gap-2"><button className="button-secondary" onClick={()=>{setVersionId(v.id);setTab("原文预览");}}>阅读此版本</button>{v.lifecycle_status==="pending_review"&&v.parse_status!=="failed"?<button className="button-primary" onClick={()=>void govern(v,"review")}><CheckCircle2 size={15}/>审核通过</button>:null}{v.lifecycle_status==="approved"?<button className="button-primary" onClick={()=>void govern(v,"activate")}><PlayCircle size={15}/>激活生效</button>:null}</div></li>)}</ul>:<p>暂无版本记录</p>}</section>:null}
      </div>
    </main>
  );
}

...
export type AssetDisplay = {
  ...
  qualified_technical_name?: string | null;
...
export type LineageNodeDisplay = AssetDisplay;
export type LineageNode = { id:number;node_type:string;logical_name:string;database_name?:string|null;schema_name?:string|null;table_name?:string|null;column_name?:string|null;catalog_column_id?:number|null;source_field_id?:number|null;mart_field_id?:number|null;target_field_id?:number|null;unresolved_flag:boolean;metadata:Record<string,unknown>;display?:LineageNodeDisplay|null };
...
export type LineageTraversal = {
  mode:"bounded_traversal"|"revision_snapshot"|"compatibility_facts";
  root?:{ root_type:string;root_id:number;node_id?:number;node_ids?:number[] } | null;
  direction:string;
  depth:number;
  depth_reached:number;
  view:"business"|"technical";
  direction_applied:boolean;
  depth_applied:boolean;
  ...
  layer_filtered_node_count:number;
  dangling_edge_count:number;
};
export type LineageGraph = { nodes:LineageNode[];edges:LineageEdge[];direction:string;depth:number;view?:"business"|"technical";traversal?:LineageTraversal;truncated:boolean };
// Phase D 端到端路径契约（GET /projects/{id}/lineage/path）。图形、表格和导出必须
// 消费同一份响应，避免页面各自拼接标签或实体。
export type LineagePathNode = {
  ...
  current_display?:AssetDisplay|null;
  layer_code:string;
  layer_name:string;
  technical_identifier?:string|null;
  unresolved_flag:boolean;
  resolution_status:string;
  lineage_node_ids?:number[];
  revision_node_keys?:string[];
  script_file_version_ids?:number[];
  metadata?:Record<string,unknown>;
};
export type LineagePathEdge = {
  id:string;
...
export type LineageResolvedPath = {
  path_id:string;
  root_node_id:string;
...
export type LineageGapRecommendation = {
  gap_type:string;
...
export type LineagePathResponse = {
  project_id:number;
  root:{ entity_type:string; entity_id:number; node_id:string; display:AssetDisplay };
  ...
  paths:LineageResolvedPath[];
  unresolved_nodes?:LineagePathNode[];
  gap_recommendations?:LineageGapRecommendation[];
  truncated:boolean;
  warnings?:string[];
  confidence:string;
  evidence_refs?:Record<string,unknown>[];
...
export type SemanticLifecycleStatus = "draft" | "ai_suggested" | "confirmed" | "rejected" | "deprecated";
export type SemanticBindingEntityType = "target_table" | "target_field" | "mart_table" | "mart_field" | "source_table" | "source_field" | "scenario" | "knowledge_unit" | "source_to_mart_mapping" | "mart_to_ybt_mapping" | "scenario_business_mapping" | "scenario_technical_lineage";
export type SemanticCatalogEffectiveVersion = { id:number;version_no:number;concept_name:string;definition?:string|null;aliases:string[];business_domain?:string|null;owner_department?:string|null;status:"confirmed";source_type:string;source_id?:number|null;confirmed_by?:string|null;confirmed_at?:string|null;effective_from:string;effective_to?:string|null;updated_at:string };
export type SemanticCatalogReviewSummary = { pending:boolean;pending_count:number;task_id?:number|null;status?:string|null;current_step?:string|null };
export type ReadableSemanticAssetReference = { entity_type:SemanticBindingEntityType;restricted:false;entity_id:number;display_name:string;display_code?:string|null;href?:string|null;target_field_id?:number|null;document_id?:number|null };
export type RestrictedSemanticAssetReference = { entity_type:SemanticBindingEntityType;restricted:true };
export type SemanticAssetReference = ReadableSemanticAssetReference | RestrictedSemanticAssetReference;
